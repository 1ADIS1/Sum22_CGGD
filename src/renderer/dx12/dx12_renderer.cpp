...
void cg::renderer::dx12_renderer::create_pso(const std::string& shader_name)
{
	...
	// Shadow
	ComPtr<ID3DBlob> vertex_shadow_map = compile_shader(
			get_shader_path(shader_name),
			"VSShadowMap",
			"vs_5_0"
	);
	pso_desc.PS = CD3DX12_SHADER_BYTECODE(0, 0);
	pso_desc.VS = CD3DX12_SHADER_BYTECODE(vertex_shadow_map.Get());
...
void cg::renderer::dx12_renderer::load_assets()
{
	...
	// CBV, null texture, shadow map, textures
	...
	null_srv_desc.Format = DXGI_FORMAT_R32_FLOAT;
	...
	device->CreateShaderResourceView(shadow_map.Get(),
									 &shadow_srv_desc,
	...
	for (size_t i = 0; i < model->get_index_buffers().size(); i++)
	{
		...
		create_resource_on_default_heap(
				textures[i],
				0,
				model->get_per_shape_texture_files()[i].wstring(),
				&texture_desc
				);
		...
		create_shader_resource_view(
				textures[i],
				cbv_srv_heap.get_cpu_descriptor_handle(static_cast<UINT>(i + 3))
				);
	}
...
void cg::renderer::dx12_renderer::populate_command_list()
{
	...
	command_list->SetGraphicsRootDescriptorTable(
			2,
			cbv_srv_heap.get_gpu_descriptor_handle(2));
	...
	for (size_t s = 0; s < model->get_vertex_buffers().size(); ++s) {
		...
		if(!model->get_per_shape_texture_files()[s].empty())
		{
			...
			command_list->SetGraphicsRootDescriptorTable(
					1,
					cbv_srv_heap.get_gpu_descriptor_handle(static_cast<UINT>(s + 3))
					);
		}
